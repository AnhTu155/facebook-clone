import React from "react";

const MoreOneIcon: React.FC = () => (
  // <svg
  //   className="w-[25px] h-[25px] text-gray-800"
  //   aria-hidden="true"
  //   viewBox="0 0 24 24"
  // >
  //   <path d="m12 21.35-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54z"></path>
  // </svg>

  <img
    srcSet="https://img.icons8.com/?size=48&id=DFU1kReSUccu&format=png 1x, https://img.icons8.com/?size=96&id=DFU1kReSUccu&format=png 2x"
    width="48"
    height="48"
    alt="Heart icon"
    className="w-[28px] h-[28px] text-gray-800"
    loading="lazy"
  />
);

export default MoreOneIcon;
